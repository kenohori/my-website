...
Creating your own Xcode projects for CGAL is easy, but I never found a post that properly explains how to get CGAL working correctly on your Mac and in Xcode, so I figured it all by trial and error. This is my attempt to make like easier for other Mac CGAL users.
...
Some additional dependencies might work by passing additional parameters to `brew install cgal`, such as `--with-imaging` to install Qt. But I've had very mixed luck with that. Especially, the current situation with Qt5 is not good.
...
Next, go to the **Build Phases** tab of the same target. Here, you should add all the dynamic libraries that your code will need to link to. So, open the **Link Binary with Libraries** collapsible menu and click the **+** sign.
...
In there, you should select all the libraries that your CGAL packages need. For the basic stuff, these would be:
...
With this kind of configuration, you should be able to compile any CGAL code. Just as an example, let's substitute the contents of `main.cpp` with:
...
If you thing anything wrong with these instructions or if you have any better ideas, [let me know]({{ site.baseurl }}/en/contact/).
